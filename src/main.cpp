...
#include "experiment-biased_natural_freqs.hpp"
...
"Chi Curve: run a sequence of batches, each at a different coupling strength."
"Each batch consists of TRIAL number of trials, each running for ITERS iterations.\n\n"
...
"--initial-condition (string) [random/uniform/wave[n]]  sets how to initialize the states array\n\n"
"TRIAL TIME EVOLUTION parameters\n"
"Invoke this experiment with the flag -t\n"
"--log-phases (integer) [default 0] if > 1, for oscillators groups of this size and only log the most frequent phase to the output file\n"
"--log-interval (real)  [default 10*dtmin] minimum simulation time passed between two consecutive logs of the phases\n"
"--discard-burn (bool)  [default 0] if true, don't record the burn iterations to the output file\n"
...
"BIASED NATURAL FREQUENCIES parameters\n"
"Invoke this experiment with the flag -n\n"
"--log-interval (real)          [default 10*dtmin] minimum simulation time passed between two consecutive logs\n"
"--discard-burn (bool)          [default 0] if true, don't record the burn iterations to the output file\n"
"-tc (real)                     [default 2.0] coupling strength for trial\n"
"-ts (uint)                     [default " + std::to_string(DEFAULT_SEED) + "] trial seed\n"
"-tr (uint)                     [default " + std::to_string(DEFAULT_STREAM) + "] trial stream\n"
...
"-tb (int)                      [default 3*N*log(N)] number of burn iterations in trial\n"
"-tf (string)                   [default autogenerated] trial file name\n"
"--nfreqs_bias_amplitude (real) [default 0.5] amplitude of bias on the natural frequencies\n"
"--nfreqs_bias (real)           [default \"sinesqr\"] wave type of the bias on the natural frequencies\n\n";

        //if (cmdOptionExists(argv, argv+argc, "--nfreqs_bias")) {
        //    opt = getCmdOption(argv, argv+argc, "--nfreqs_bias");
        //    if ( (opt=="sinesqr") || (opt=="linear") ) {
        //        _nfreq_bias = opt;
        //    }
        //}
        //if (cmdOptionExists(argv, argv+argc, "--nfreqs_bias_amplitude")) {
        //    opt = getCmdOption(argv, argv+argc, "--nfreqs_bias_amplitude");
    ...
    // RUN A BIASED NATURAL FREQUENCIES EXPERIMENT
    if (cmdOptionExists(argv, argv+argc, "-n")) {
        Biased_natural_freqs exp(argc, argv);
        exp.print_headers();
        exp.run();
    }
